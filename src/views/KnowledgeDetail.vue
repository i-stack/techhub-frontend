...
export default {
	...
	methods: {
		async loadKnowledgeDetail() {
			...
			try {
				// 从路由状态中获取传递的数据
				const knowledgeData = this.$router.currentRoute.value.state?.knowledgeData

				if (knowledgeData) {
					this.knowledge = knowledgeData
				} else {
					// 如果没有传递数据，尝试调用 API
					// TODO: 调用真实的 API 获取知识详情
					// const response = await knowledgeAPI.getKnowledgeDetail(this.id)
					// this.knowledge = response.data

					// 临时：显示错误信息，提示需要实现 API
					this.$message.error('知识详情数据未找到，API 尚未实现')
					this.$router.go(-1)
				}
			} catch (error) {
